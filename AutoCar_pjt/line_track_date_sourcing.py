    ...
    def save_image(event, x, y, flags, param):
        ...
        # 저장된 사진을 새로운 창에 띄우기
            ...
            if captured_image is not None:
                cv2.imshow('Captured Image', cv2.circle(captured_image, (x, y), 5, (0, 255, 0), 2))  # 새로운 창에 이미지 표시
            else:
                print("이미지를 불러오는 데 실패했습니다.")

    # 마우스 클릭 이벤트 등록
    cv2.namedWindow('Camera Feed')  # 창 이름 지정
    cv2.setMouseCallback('Camera Feed', save_image)
